...
backend_MusteR <- function(filename){
...
if(a_if){
  ...
  pdb = list()
  #problema estava aqui abaixo
  #Error in read.pdb: unused arguments (maxlines = maxlines,
  #multi = multi, rm.insert = rm.insert, rm.alt = rm.alt, ATOM.only = ATOM.only, verbose = verbose)
  browser()
...
if (a_if){
  ...
  pdb = align_db_list(pdb,ids=1:length(pdb),pdbpath,workpath,prodpath,vcores=c(ncores=0,ncores=0),pre.sufix=pre.sufix,
...
if(a_if){
  ...
  for (i in seq.id ){ #i = 4;
    ...
    pdb.out[[j]] = write_molecule_muster_version(pdb[[j]], dba2[[1]][[i]], 
                                                 area.lim = 100)
    j = j + 1
  }
  dba2.size = object.size(dba2)
...
if(a_if){
  ...
  # seq.id = c(4)
  # m.ligs2 = consolidate_res_table(dba2,seq.id)
  # seq.id = c(2,4)
  # m.ligs3 = consolidate_res_table(dba2,seq.id)
...
if(0){
  library(orca)
  p.sizes = c(1200,600)
  orca(p.all,"FIGs/test-v1.png",scale=2,width=p.sizes[1],height=p.sizes[2])
}
